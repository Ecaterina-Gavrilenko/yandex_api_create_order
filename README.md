...
- Валидация формата трека (должен быть числом)

- Сохранение для использования в следующем шаге

3. Получение заказа по треку:

- Отправка GET-запроса с параметром track

- Использование сохраненного номера трека
...
- В файле configuration вставьте URL. 

- В файле sender_stand_request.py нажмите кнопку Run.

Эти шаги позволят отправить запрос к указанному стенду и запустить тесты.
